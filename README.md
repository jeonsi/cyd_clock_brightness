...
- **탭/스와이프 판정** — 스와이프는 원시 좌표의 최대 변위(80px 이상, 빠른 플릭도 놓치지 않음), 탭은 EMA 필터된 좌표의 최대 변위(60px 미만)와 **프레임당 속도**(6px/10ms 미만)로 판정. 그 사이는 끊기거나 늦게 등록된 스와이프로 보고 무시. **눌림 보류(touch slop, 70ms)**: 접촉 확정 후 잠시 LVGL에 눌림을 전달하지 않고, 그 안에 움직이면 제스처로 확정해 버튼이 눌리지 않은 채 스와이프가 되고, 움직이지 않으면 그때 눌림을 전달. 눌림이 전달된 뒤(버튼을 누른 채) 움직인 경우는 제스처로 보지 않아 버튼 조작이 스와이프로 새지 않음
...
| `TOUCH_SWIPE_MIN_PX` | 80 | 스와이프로 인식할 최소 이동 거리(px). 속도 무관, 누른 동안의 최대 변위로 판정 |
