...
- **여섯 개의 화면(스와이프 순환)** — 좌우로 스와이프하면 **디지털 → 아날로그 → 달력 → 스톱워치 → 타이머 → 알람** 순으로 전환되고(반대 방향은 역순), 선택이 NVS에 저장됨. 탭은 여전히 밝기 슬라이더(버튼 위 탭 제외)
  ...
  - **달력 화면**: 월간 달력. 일요일·공휴일은 빨강, 토요일은 파랑, 오늘은 주황 배경으로 강조. 배경 테마의 명암에 따라 글자색이 자동 조정됨. **위/아래로 스와이프하면 다음/이전 달**을 넘겨볼 수 있고(±120개월), 다른 화면으로 나가면 이번 달로 복귀 (오늘 강조는 실제 이번 달에서만 표시)
...
- **압력 히스테리시스** — 터치 시작은 `TOUCH_Z_PRESS`(50) 이상 한 프레임(그보다 가벼운 40~49 접촉은 `TOUCH_LIGHT_PRESS_FRAMES` 2프레임 지속 시 시작). 그 뒤로는 `TOUCH_Z_RELEASE`(40)만 유지하면 됨. 펜촉의 약한 압력이나 살짝 누른 스와이프도 끝까지 추적됨
...
- **눌림 확정 지연** — 접촉 첫 프레임(압력이 낮아 좌표가 튀는 구간)은 건너뛰고 다음 프레임 좌표를 누른 지점으로 사용 → 인접 버튼 오타깃 방지 (`TOUCH_PRESS_CONFIRM_FRAMES`). 확정 전에 끝난 아주 짧은 탭(1프레임)도 압력이 `TOUCH_Z_SHORT_TAP`(100) 이상이면 그 좌표로 눌림/릴리즈를 합성해 잃지 않음(가벼운 1프레임 스침은 노이즈로 간주). 화면의 스크롤 플래그를 꺼서 눌린 채 흔들려도 버튼 클릭이 스크롤로 취소되지 않음
- 100 ms 릴리즈 디바운스 — 저항막 패널 특유의 드래그 중 순간 접촉 끊김을 흡수
- **탭/스와이프 판정** — 스와이프는 원시 좌표의 최대 변위(60px 이상, 빠른 플릭도 놓치지 않음), 탭은 EMA 필터된 좌표의 최대 변위(30px 미만)와 **프레임당 속도**(6px/10ms 미만)로 판정. 그 사이는 끊기거나 늦게 등록된 스와이프로 보고 무시. **살짝 닿은 사전 접촉의 첫 좌표를 제스처 시작점**으로 써서, 가볍게 시작한 스와이프도 실제 출발점부터 측정됨. **눌림 보류(touch slop, 70ms)**: 접촉 확정 후 잠시 LVGL에 눌림을 전달하지 않고, 그 안에 움직이면 제스처로 확정해 버튼이 눌리지 않은 채 스와이프가 되고, 움직이지 않으면 그때 눌림을 전달. 눌림이 전달된 뒤(버튼을 누른 채) 움직인 경우는 제스처로 보지 않아 버튼 조작이 스와이프로 새지 않음
...
| `TOUCH_PRESS_HOLD_MS` | 70 | 눌림 보류 시간. 이 안에 움직이면 제스처, 아니면 눌림 전달 |
...
| `TOUCH_MOVE_STEP_PX` | 6 | 탭으로 인정할 최대 프레임당 이동(px). 그 이상 빠르면 총 이동이 짧아도 탭 아님 |
| `TOUCH_TAP_GUARD_MS` | 300 | 스와이프 직후 클릭을 무시하는 시간 |
| `TOUCH_LIGHT_PRESS_FRAMES` | 2 | 시작 문턱 미만의 가벼운 접촉이 이 프레임 수 이상 지속되면 눌림으로 인정 |
| `TOUCH_PRE_CONTACT_MS` | 200 | 가벼운 사전 접촉의 첫 좌표를 제스처 시작점으로 쓰는 시간 한도 |
| `TOUCH_TAP_MAX_PX` | 30 | 탭으로 인정할 최대 이동(px, 필터된 좌표 기준). 그 이상~스와이프 미만은 끊긴 스와이프로 간주해 무시 |
| `TOUCH_Z_SHORT_TAP` | 100 | 1프레임짜리 짧은 탭을 클릭으로 인정할 최소 압력(고스트 터치 방지) |
| `TOUCH_Z_PRESS/RELEASE` | 50/40 | 터치 시작/유지 압력 임계값. 유령 터치가 생기면 PRESS를 올리고, 펜 드래그가 끊기면 RELEASE를 내림 |
