...
- **12/24시간 형식** — 밝기 패널 우측 상단의 `12H`/`24H` 버튼으로 전환, NVS에 저장. 24시간 모드는 "09:05"처럼 두 자리 시로 표시되고 AM/PM이 사라짐
...
- **다섯 개의 화면(스와이프 순환)** — 좌우로 스와이프하면 **디지털 → 아날로그 → 달력 → 스톱워치 → 타이머** 순으로 전환되고(반대 방향은 역순), 선택이 NVS에 저장됨. 탭은 여전히 밝기 슬라이더(버튼 위 탭 제외)
  - **아날로그 화면**: 왼쪽에 시침·분침·초침(주황)이 있는 다이얼(LVGL `lv_scale`), 오른쪽에 AM/PM(24시간 모드에서는 생략)·요일·날짜·음력·절기·명절 정보를 세로로 표시. 분침은 초에 비례해 매초 0.1°씩 서서히 전진하고 시침은 매분 0.5°씩 이동
