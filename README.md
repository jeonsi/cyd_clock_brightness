...
- **압력 히스테리시스** — 터치 시작은 `TOUCH_Z_PRESS`(100) 이상 한 프레임(그보다 가벼운 50~99 접촉은 `TOUCH_LIGHT_PRESS_FRAMES` 3프레임 지속 시 시작). 그 뒤로는 `TOUCH_Z_RELEASE`(50)만 유지하면 됨. 펜촉의 약한 압력이나 살짝 누른 스와이프도 끝까지 추적됨
...
- **눌림 확정 지연** — 접촉 첫 프레임(압력이 낮아 좌표가 튀는 구간)은 건너뛰고 다음 프레임 좌표를 누른 지점으로 사용 → 인접 버튼 오타깃 방지 (`TOUCH_PRESS_CONFIRM_FRAMES`). 확정 전에 끝난 아주 짧은 탭(1프레임)도 압력이 `TOUCH_Z_SHORT_TAP`(150) 이상이면 그 좌표로 눌림/릴리즈를 합성해 잃지 않음(가벼운 1프레임 스침은 노이즈로 간주). 화면의 스크롤 플래그를 꺼서 눌린 채 흔들려도 버튼 클릭이 스크롤로 취소되지 않음
...
| `TOUCH_LIGHT_PRESS_FRAMES` | 3 | 시작 문턱 미만의 가벼운 접촉이 이 프레임 수 이상 지속되면 눌림으로 인정 |
...
| `TOUCH_Z_SHORT_TAP` | 150 | 1프레임짜리 짧은 탭을 클릭으로 인정할 최소 압력(고스트 터치 방지) |
