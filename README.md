...
- **다섯 개의 화면(스와이프 순환)** — 좌우로 스와이프하면 **디지털 → 아날로그 → 달력 → 스톱워치 → 타이머** 순으로 전환되고(반대 방향은 역순), 선택이 NVS에 저장됨. 탭은 여전히 밝기 슬라이더(버튼 위 탭 제외)
  ...
  - **스톱워치 화면**: 분:초를 DSEG 대형 폰트로, 1/100초를 옆에 작게 표시(표시 상한 99:59). 실행 중에는 전용 30ms 타이머로 마지막 자리가 실제 스톱워치처럼 굴러가고, 정지하면 ms 단위 측정값이 그대로 고정됨. ▶/⏸ 버튼으로 시작·정지, **LAP 버튼으로 랩타임 기록 — 각 줄에 랩 번호·구간 시간·누적 시간을 표시, 최근 3개를 최신순으로. 정지(⏸)를 누르면 마지막 구간도 랩으로 자동 기록**, ↻ 버튼으로 리셋(랩도 함께 지움). 다른 화면으로 이동해도 백그라운드에서 계속 측정됨
