...
- **압력 히스테리시스** — 터치 시작은 `TOUCH_Z_PRESS`(200) 이상, 시작 뒤에는 `TOUCH_Z_RELEASE`(150)만 유지하면 됨(순수 히스테리시스 — 유지 문턱은 시작 판정에 관여하지 않음). 펜촉의 약한 압력이나 살짝 누른 스와이프도 끝까지 추적됨
...
| `TOUCH_Z_PRESS/RELEASE` | 200/150 | 터치 시작/유지 압력 임계값. 유령 터치가 생기면 PRESS를 올리고, 펜 드래그가 끊기면 RELEASE를 내림 |
