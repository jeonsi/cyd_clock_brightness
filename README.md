...
- **배경색 선택** — 밝기 패널 우측 상단의 스와치 4개(흰색·아이보리·다크그레이·검정)로 배경을 바꿀 수 있고 선택이 NVS에 저장됨. 어두운 배경에서는 다이얼 눈금·시침/분침·부팅 문구 색이 자동으로 밝게 조정됨(소스의 `THEMES[]` 테이블에서 색상 추가/수정 가능)
...
- **아날로그 화면** — 좌우로 스와이프하면 디지털 ↔ 아날로그 화면이 전환되고 선택이 NVS에 저장됨. 아날로그 화면은 왼쪽에 시침·분침·초침(주황)이 있는 다이얼(LVGL `lv_scale`), 오른쪽에 AM/PM·요일·날짜·음력·절기·명절 정보를 세로로 표시. 분침은 초에 비례해 매초 0.1°씩 서서히 전진하고 시침은 매분 0.5°씩 이동. 탭은 여전히 밝기 슬라이더
