...
- **배경색 선택** — 밝기 패널의 스와치 16개(밝은 줄: 흰색·아이보리·로즈·라벤더·미스트블루·세이지·샌드·미드그레이 / 어두운 줄: 슬레이트블루·틸·올리브·플럼·와인·브라운·네이비·검정)로 배경을 바꿀 수 있고 선택이 NVS에 저장됨. 어두운 배경에서는 다이얼 눈금·시침/분침·부팅 문구 색이 자동으로 밝게 조정됨(소스의 `THEMES[]` 테이블에서 색상 추가/수정 가능)
