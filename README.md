...
- **눌림 확정 지연** — 접촉 첫 프레임(압력이 낮아 좌표가 튀는 구간)은 건너뛰고 다음 프레임 좌표를 누른 지점으로 사용 → 인접 버튼 오타깃 방지 (`TOUCH_PRESS_CONFIRM_FRAMES`)
- 100 ms 릴리즈 디바운스 — 저항막 패널 특유의 드래그 중 순간 접촉 끊김을 흡수
- **탭/스와이프 판정** — 스와이프는 원시 좌표의 최대 변위(60px 이상, 빠른 플릭도 놓치지 않음), 탭은 EMA 필터된 좌표의 최대 변위(24px 미만, 누르고 뗄 때의 한 프레임 튐에 영향 없음)로 판정. 그 사이 구간은 도중에 끊긴 스와이프로 보고 무시
