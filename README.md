...
- **배경색 선택** — 밝기 패널의 스와치 8개(흰색·아이보리·연회색·페일블루·다크그레이·네이비·다크브라운·검정)로 배경을 바꿀 수 있고 선택이 NVS에 저장됨. 어두운 배경에서는 다이얼 눈금·시침/분침·부팅 문구 색이 자동으로 밝게 조정됨(소스의 `THEMES[]` 테이블에서 색상 추가/수정 가능)
